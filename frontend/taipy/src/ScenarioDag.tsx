...
import { PyForgeDiagramModel } from "./projectstorm/models";
...
const ScenarioDag = (props: ScenarioDagProps) => {
    ...
    useEffect(() => {
        const model = new PyForgeDiagramModel(onClick);
        initDiagram(engine);
        let doLayout = false;
        if (displayModel) {
        ...
        const rects =
            engine.getModel() &&
            (engine.getModel() as PyForgeDiagramModel).scenarioId == model.scenarioId &&
            engine
                .getModel()
                .getNodes()
                .reduce((pv, nm) => {
