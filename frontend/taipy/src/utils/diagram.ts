...
import { PyForgeDiagramModel, PyForgeNodeModel } from "../projectstorm/models";
import { PyForgeNodeFactory, PyForgePortFactory } from "../projectstorm/factories";
...
export const initDiagram = (engine: DiagramEngine) => {
    nodeTypes.forEach((nodeType) => engine.getNodeFactories().registerFactory(new PyForgeNodeFactory(nodeType)));
    engine.getPortFactories().registerFactory(new PyForgePortFactory());
...
export const getModelLinks = (model: PyForgeDiagramModel) => Object.values(model.getActiveLinkLayer().getLinks());
...
export const populateModel = (displayModel: DisplayModel, model: PyForgeDiagramModel) => {
