...
describe("reducer", () => {
    it("store socket connected", async () => {
        expect(
            pyforgeReducer({ ...INITIAL_STATE }, { type: "SOCKET_CONNECTED" } as PyForgeBaseAction).isSocketConnected
        ).toBeDefined();
    });
    it("returns update", async () => {
        expect(
            pyforgeReducer({ ...INITIAL_STATE }, {
                type: "UPDATE",
                name: "name",
                payload: { value: "value" },
            } as PyForgeBaseAction).data.name
        ).toBeDefined();
    });
    it("store locations", async () => {
        expect(
            ...
            } as PyForgeBaseAction).locations
        ).toBeDefined();
    });
    it("set notification", async () => {
        expect(
            pyforgeReducer({ ...INITIAL_STATE }, {
                ...
                message: "message",
                system: "system",
            } as PyForgeBaseAction).notifications
        ).toHaveLength(1);
    });
    ...
    it("set navigate", async () => {
        expect(
            pyforgeReducer({ ...INITIAL_STATE }, {
                ...
                to: "navigateTo",
                tab: "_blank",
            } as PyForgeBaseAction).navigateTo
        ).toBeDefined();
    });
    it("set client id", async () => {
        expect(pyforgeReducer({ ...INITIAL_STATE }, { type: "CLIENT_ID", id: "id" } as PyForgeBaseAction).id).toBeDefined();
    });
    it("set Acknowledgement", async () => {
        expect(
            pyforgeReducer({ ...INITIAL_STATE }, {
                ...
                id: "id",
            } as PyForgeBaseAction)
        ).toEqual(INITIAL_STATE);
    });
    it("remove Acknowledgement", async () => {
        expect(
            pyforgeReducer({ ...INITIAL_STATE, ackList: ["ack"] }, {
                type: "ACKNOWLEDGEMENT",
                id: "ack",
            } as PyForgeBaseAction)
        ).toEqual(INITIAL_STATE);
    ...
    it("set Menu", async () => {
        expect(
            pyforgeReducer({ ...INITIAL_STATE }, { type: "SET_MENU", menu: {} } as PyForgeBaseAction).menu
        ).toBeDefined();
    });
    it("sets download", async () => {
        expect(
            pyforgeReducer({ ...INITIAL_STATE }, {
                type: "DOWNLOAD_FILE",
                content: {},
            } as PyForgeBaseAction).download
        ).toBeDefined();
    });
    it("resets download", async () => {
        expect(
            pyforgeReducer({ ...INITIAL_STATE }, { type: "DOWNLOAD_FILE" } as PyForgeBaseAction).download
        ).toBeUndefined();
    });
    it("sets partial", async () => {
        expect(
            pyforgeReducer({ ...INITIAL_STATE }, {
                ...
                name: "partial",
                create: true,
            } as PyForgeBaseAction).data.partial
        ).toBeDefined();
    });
    it("resets partial", async () => {
        expect(
            pyforgeReducer({ ...INITIAL_STATE, data: { partial: true } }, {
                type: "PARTIAL",
                name: "partial",
            } as PyForgeBaseAction).data.partial
        ).toBeUndefined();
...
describe("storeBlockUi function", () => {
    ...
    it("stores message block in localStorage when document not visible", () => {
        ...
        storeBlockUi(block)();
        expect(localStorage.setItem).toHaveBeenCalledWith("PyForgeBlockUi", JSON.stringify(block));
    });
...
describe("pyforgeReducer function", () => {
    it("should not change state for SOCKET_CONNECTED action if isSocketConnected is already true", () => {
        ...
        const initialState: PyForgeState = { ...INITIAL_STATE, isSocketConnected: true };
        const newState = pyforgeReducer(initialState, action);
    ...
    it("should handle UPDATE action", () => {
        ...
        const newState = pyforgeReducer({ ...INITIAL_STATE }, action);
    ...
    it("should handle DELETE_NOTIFICATION action", () => {
        ...
        const initialState: PyForgeState = {
            ...INITIAL_STATE,
            notifications: [
                {
                    atype: "error",
    ...
    it("should not modify state if no notification are present", () => {
        const initialState: PyForgeState = { ...INITIAL_STATE, notifications: [] };
    ...
    it("should handle SET_BLOCK action", () => {
        const initialState: PyForgeState = { ...INITIAL_STATE, block: undefined };
        const action = {
            type: Types.SetBlock,
            noCancel: false,
            action: "blockAction",
    ...
    it("should handle DOWNLOAD_FILE action", () => {
        const initialState: PyForgeState = { ...INITIAL_STATE, download: undefined };
    ...
    it("should handle PARTIAL action", () => {
        const initialState: PyForgeState = { ...INITIAL_STATE, data: { test: false } };
        const actionCreate = {
            type: Types.Partial,
            name: "test",
            create: true,
        };
        let newState = pyforgeReducer(initialState, actionCreate);
        ...
        newState = pyforgeReducer(newState, actionDelete);
        expect(newState.data.test).toBeUndefined();
    });
    it("should handle MULTIPLE_UPDATE action", () => {
        const initialState: PyForgeState = { ...INITIAL_STATE, data: { test1: false, test2: false } };
        const action = {
    ...
    it("should handle SetTimeZone action with fromBackend true", () => {
        const initialState: PyForgeState = { ...INITIAL_STATE, timeZone: "oldTimeZone" };
...
describe("initializeWebSocket function", () => {
    ...
    const mockDispatch: Dispatch<PyForgeBaseAction> = jest.fn();
    beforeEach(() => {
        mockSocket = {
            on: jest.fn(),
            connect: jest.fn(),
