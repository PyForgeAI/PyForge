...
import { getUpdateVars } from "../components/PyForge/utils";
import { PageContext, PyForgeContext } from "../context/pyforgeContext";
import { createRequestUpdateAction, FormatConfig, PyForgeBaseAction } from "../context/pyforgeReducers";
...
export const useFormatConfig = (): FormatConfig => {
    const { state } = useContext(PyForgeContext);
...
export const useDispatch = () => {
    const { dispatch } = useContext(PyForgeContext);
    return dispatch;
};
