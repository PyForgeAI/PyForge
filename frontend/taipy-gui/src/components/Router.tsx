...
import UIBlocker from "./PyForge/UIBlocker";
import Navigate from "./PyForge/Navigate";
import Menu from "./PyForge/Menu";
import PyForgeNotification from "./PyForge/Notification";
import GuiDownload from "./PyForge/GuiDownload";
...
const Router = () => {
    const [state, dispatch] = useReducer(pyforgeReducer, INITIAL_STATE, pyforgeInitialize);
    const [routes, setRoutes] = useState<Record<string, string>>({});
    const refresh = !!Object.keys(routes).length;
    const themeClass = "pyforge-" + state.theme.palette.mode;
    ...
    useEffect(() => {
        ...
        axios
            .get<AxiosRouter>("pyforge-init", { params: { client_id: state.id || "", v: window.pyforgeVersion } })
            .then((result) => {
                                ...
                                <BrowserRouter>
                                    <Box style={containerSx}>
                                        ...
                                        {state.ackList.length ? (
                                            <Box sx={progressSx} className="pyforge-busy">
                                                <CircularProgress size="1em" disableShrink />
                                            </Box>
                                        ) : null}
                                    </Box>
                                    <ErrorBoundary FallbackComponent={ErrorFallback}>
                                        <PyForgeNotification notifications={state.notifications} />
