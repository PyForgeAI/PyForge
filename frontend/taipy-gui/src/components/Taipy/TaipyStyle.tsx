...
export const getComponentClassName = (children: ReactNode) =>
    (
        React.Children.map(children, (element) =>
            React.isValidElement(element) && element.type == PyForgeStyle ? (element.props as Record<string, unknown>).className : undefined
        )?.filter((v) => v) || []
    ).join(" ");
...
const PyForgeStyle = (props: PyForgeStyleProps) => {
    try {
        ...
        console.log("PyForgeStyle", props, e);
        return null;
    }
};

export default PyForgeStyle;
