...
describe("Toggle Component", () => {
    ...
    it("uses the class", async () => {
        const { getByText } = render(<Toggle lov={lov} className="pyforge-toggle" />);
        const elt = getByText("Item 1");
        expect(elt.parentElement?.parentElement?.parentElement).toHaveClass("pyforge-toggle");
    });
    ...
    describe("As Switch", () => {
        ...
        it("uses the class", async () => {
            const { getByText } = render(<Toggle isSwitch={true} label="switch" className="pyforge-toggle" />);
            const elt = getByText("switch");
            expect(elt.parentElement).toHaveClass("pyforge-toggle-switch");
        });
        it("shows a selection at start", async () => {
            const { getByText } = render(
