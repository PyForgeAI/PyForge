...
const messages: TableValueType = {
    [valueKey]: {
        data: [
            ...
            ["3", "This from the sender User", "pyforge"],
...
describe("Chat Component", () => {
    it("renders", async () => {
        ...
        const input = getByLabelText("message (pyforge)");
        expect(input.tagName).toBe("INPUT");
    });
    it("uses the class", async () => {
        const { getByText } = render(
            <Chat messages={messages} className="pyforge-chat" defaultKey={valueKey} mode="raw" />
        );
        const elt = getByText(searchMsg);
        expect(
            elt.parentElement?.parentElement?.parentElement?.parentElement?.parentElement?.parentElement
        ).toHaveClass("pyforge-chat");
    });
    ...
    it("is disabled", async () => {
        ...
        const elt = getByLabelText("message (pyforge)");
    ...
    it("can hide input", async () => {
        render(<Chat messages={messages} withInput={false} className="pyforge-chat" defaultKey={valueKey} mode="raw" />);
        const elt = document.querySelector(".pyforge-chat input");
        expect(elt).toBeNull();
    });
    it("renders markdown by default", async () => {
        render(<Chat messages={messages} className="pyforge-chat" defaultKey={valueKey} />);
        const elt = document.querySelector(".pyforge-chat .pyforge-chat-received .MuiPaper-root");
        await waitFor(() => expect(elt?.querySelector("p")).not.toBeNull());
    });
    it("can render pre", async () => {
        const { getByText } = render(
            <Chat messages={messages} defaultKey={valueKey} className="pyforge-chat" mode="pre" />
        );
        const elt = getByText(searchMsg);
        expect(elt.tagName).toBe("PRE");
        expect(elt.parentElement).toHaveClass("pyforge-chat-pre");
    });
    it("can render raw", async () => {
        const { getByText } = render(
            <Chat messages={messages} defaultKey={valueKey} className="pyforge-chat" mode="raw" />
        );
        const elt = getByText(searchMsg);
        expect(elt).toHaveClass("pyforge-chat-raw");
    });
    it("dispatch a well formed message by Keyboard", async () => {
        ...
        expect(dispatch).toHaveBeenCalledWith({
            ...
            payload: {
                action: undefined,
                args: ["Enter", "varName", "new message", "pyforge", null],
            },
        });
    });
    it("dispatch a well formed message by button", async () => {
        ...
        expect(dispatch).toHaveBeenCalledWith({
            ...
            payload: {
                action: undefined,
                args: ["click", "varName", "new message", "pyforge", null],
            },
        });
    });
            ...
            expect(dispatch).toHaveBeenCalledWith(
                expect.objectContaining({
                    ...
                    payload: expect.objectContaining({
                        args: ["click", "varName", "Test message with image", "pyforge", "mocked-url"],
                    }),
                })
            );
        });
