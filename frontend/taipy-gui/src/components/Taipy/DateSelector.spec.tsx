...
describe("DateSelector Component", () => {
    ...
    it("displays the right info for string", async () => {
        const { getByTestId } = render(
            <LocalizationProvider dateAdapter={AdapterDateFns}>
                <DateSelector date={curDateStr} defaultDate="2001-01-01T00:00:01.001Z" className="pyforge-date" />
            </LocalizationProvider>
        );
        const elt = getByTestId("CalendarIcon");
        expect(elt.parentElement?.parentElement?.parentElement?.parentElement).toHaveClass("pyforge-date-picker");
        expect(elt.parentElement?.parentElement?.parentElement?.parentElement?.parentElement).toHaveClass("pyforge-date");
    });
...
describe("DateSelector with time Component", () => {
    ...
    it("displays the right info for string", async () => {
        const { getByTestId } = render(
            <LocalizationProvider dateAdapter={AdapterDateFns}>
                <DateSelector date={curDateStr} withTime={true} className="pyforge-time" />
            </LocalizationProvider>
        );
        const elt = getByTestId("CalendarIcon");
        expect(elt.parentElement?.parentElement?.parentElement?.parentElement).toHaveClass("pyforge-time-picker");
        expect(elt.parentElement?.parentElement?.parentElement?.parentElement?.parentElement).toHaveClass("pyforge-time");
    });
    it("displays the default value", async () => {
        render(
            <LocalizationProvider dateAdapter={AdapterDateFns}>
