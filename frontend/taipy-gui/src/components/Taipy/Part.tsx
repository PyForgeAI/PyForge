...
import { expandSx, getCssSize, PyForgeBaseProps } from "./utils";
import { PyForgeContext } from "../../context/pyforgeContext";
import { getComponentClassName } from "./PyForgeStyle";

interface PartProps extends PyForgeBaseProps {
    render?: boolean;
    ...
    return render ? (
        <Box id={id} className={`${className} ${getComponentClassName(props.children)}`} sx={boxSx}>
            {iFrame ? (
                ...
                <PyForgeRendered path={"/" + page} partial={partial} fromBlock={true} />
            ) : null}
            {props.children}
        </Box>
    ) : null;
