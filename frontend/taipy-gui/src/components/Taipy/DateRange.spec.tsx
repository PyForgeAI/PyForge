...
import { PyForgeState, INITIAL_STATE } from "../../context/pyforgeReducers";
...
describe("DateRange Component", () => {
    ...
    it("displays the right info for string", async () => {
        const { getAllByTestId } = render(
            <LocalizationProvider dateAdapter={AdapterDateFns}>
                <DateRange dates={curDates} defaultDates={defaultDates} className="pyforge-date-2" />
            </LocalizationProvider>
        );
        const elts = getAllByTestId("CalendarIcon");
        expect(elts).toHaveLength(2);
        expect(elts[0].parentElement?.parentElement?.parentElement?.parentElement).toHaveClass(
            "pyforge-date-2-picker",
            "pyforge-date-2-picker-start"
        );
        ...
        render(
            <LocalizationProvider dateAdapter={AdapterDateFns}>
                <DateRange
                    ...
                    className="pyforge-date-range"
                />
            </LocalizationProvider>
        );
        const input = document.querySelector(".pyforge-date-range-picker-start input") as HTMLInputElement;
        expect(input).toBeInTheDocument();
        expect(cleanText(input?.value || "")).toEqual("01/01/2001");
        const input2 = document.querySelector(".pyforge-date-range-picker-end input") as HTMLInputElement;
    ...
    it("is disabled", async () => {
        render(
            <LocalizationProvider dateAdapter={AdapterDateFns}>
                <DateRange dates={curDates} active={false} className="pyforge-date-range" />
            </LocalizationProvider>
        );
        const input = document.querySelector(".pyforge-date-range-picker-start input");
        expect(input).toBeInTheDocument();
        expect(input).toBeDisabled();
        const input2 = document.querySelector(".pyforge-date-range-picker-end input");
        expect(input2).toBeInTheDocument();
    ...
    it("dispatch a well formed message", async () => {
        ...
        render(
            <PyForgeContext.Provider value={{ state, dispatch }}>
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                    <DateRange dates={curDates} className="pyforge-date-range" />
                </LocalizationProvider>
...
describe("DateRange with time Component", () => {
    ...
    it("displays the right info for string", async () => {
        const { getAllByTestId } = render(
            <LocalizationProvider dateAdapter={AdapterDateFns}>
                <DateRange dates={curDates} withTime={true} className="pyforge-time" />
            </LocalizationProvider>
        );
        const elts = getAllByTestId("CalendarIcon");
        expect(elts).toHaveLength(2);
