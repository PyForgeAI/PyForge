...
import { PyForgeState, INITIAL_STATE, BlockMessage } from "../../context/pyforgeReducers";
...
describe("UIBlocker Component", () => {
    ...
    it("displays the right className", async () => {
        const { getByText } = render(<UIBlocker block={blockWithCancel} />);
        const elt = getByText("Cancel");
        expect(elt.parentElement).toHaveClass("pyforge-UIBlocker");
    });
    ...
    it("dispatch a well formed message", async () => {
        ...
        const { getByText } = render(<PyForgeContext.Provider value={{ state, dispatch }}>
