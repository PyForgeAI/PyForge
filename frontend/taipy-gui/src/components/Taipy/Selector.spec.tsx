...
describe("Selector Component", () => {
    ...
    it("uses the class", async () => {
        const { getByText } = render(<Selector lov={lov} className="pyforge-selector" />);
        const elt = getByText("Item 1");
    ...
    describe("Selector Component radio mode", () => {
        ...
        it("displays a list of unselected radios", async () => {
            const { getByText, getByRole } = render(<Selector lov={lov} mode="radio" className="pyforge-selector" />);
            getByText("Item 1");
            getByRole("radiogroup");
            expect(document.querySelector("div.pyforge-selector-radio-group")).not.toBeNull();
        });
    ...
    describe("Selector Component check mode", () => {
        ...
        it("displays a list of unselected checks", async () => {
            const { getByText } = render(<Selector lov={lov} mode="check" className="pyforge-selector" />);
            const elt = getByText("Item 1");
            expect(elt.parentElement?.parentElement).toHaveClass("pyforge-selector-check-group");
