...
describe("AutoLoadingTable Component", () => {
    ...
    it("displays the right info for class", async () => {
        const { getByText } = render(
            <AutoLoadingTable data={undefined} defaultColumns={tableColumns} className="pyforge-table" />
        );
        const elt = getByText("Entity").closest("table");
        expect(elt?.parentElement?.parentElement?.parentElement).toHaveClass("pyforge-table", "pyforge-table-autoloading");
    });
