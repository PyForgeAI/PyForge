...
describe("NavBar Component", () => {
    ...
    it("uses the class", async () => {
        const {getByText} = render(<BrowserRouter><NavBar lov={lov} className="pyforge-navbar" /></BrowserRouter>);
        const elt = getByText("Item 1");
        expect(elt.parentElement?.parentElement?.parentElement?.parentElement).toHaveClass("pyforge-navbar");
    })
    ...
    it("shows a default list when no lov", async () => {
        ...
        const state: PyForgeState = {...INITIAL_STATE, locations: {"/": "/routeloc", "/page1": "/loc1", "/page2": "/loc2"}};
        const { getByText, queryAllByRole } = render(<PyForgeContext.Provider value={{ state, dispatch }}>
