...
import { createNavigateAction } from "../../context/pyforgeReducers";
...
const Navigate = ({ to, params, tab, force }: NavigateProps) => {
    const { dispatch, state } = useContext(PyForgeContext);
        ...
        if (to) {
            ...
            if (Object.keys(state.locations || {}).some((route) => tos === route)) {
                ...
                if (force && location.pathname === navigatePath  && searchParamsLocation.toString() === searchParams.toString()) {
                    ...
                    if (tprh !== undefined) {
                        axios.post(`pyforge-rh`, { tprh, is_secure: window.location.protocol.includes("https") }).then(() => {
                            localStorage.setItem("tprh", tprh);
                            navigate(0);
                        }).catch((error) => {
                            console.error(
                                "Cannot resolve resource handler. Route `/pyforge-rh` might be missing.",
                                error,
                            );
                        });
                    }
