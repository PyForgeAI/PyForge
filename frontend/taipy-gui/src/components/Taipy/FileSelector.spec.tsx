...
describe("FileSelector Component", () => {
    ...
    it("displays the right info for string", async () => {
        const { getByText } = render(<FileSelector label="toto" defaultLabel="titi" className="pyforge-file-selector" />);
        const elt = getByText("toto");
        expect(elt.parentElement?.parentElement).toHaveClass("pyforge-file-selector");
    });
    ...
    it("checks if notification is dispatched on file upload completion", async () => {
        ...
        const { getByLabelText } = render(
            <PyForgeContext.Provider value={{ state: INITIAL_STATE, dispatch: mockDispatch }}>
