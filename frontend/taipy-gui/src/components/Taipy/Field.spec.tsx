...
describe("Field Component", () => {
    ...
    it("displays the right info for string", async () => {
        const { getByText } = render(<Field value="toto" defaultValue="titi" className="pyforge-field" />);
        const elt = getByText("toto");
        expect(elt).toHaveClass("pyforge-field");
    });
    ...
    it("can render markdown", async () => {
        const { container, getByText, findByText } = render(<Field value="titi" className="pyforge-text" mode="md" />);
    ...
    it("can render pre", async () => {
        const { container } = render(<Field value="titi" className="pyforge-text" mode="pre" />);
        const elt = container.querySelector("pre.pyforge-text-pre");
        expect(elt).toBeInTheDocument();
    });
    describe("latex mode", () => {
        it("renders LaTeX as block math", async () => {
            const { container, getByText } = render(
                <Field value={"$$x = y + 1$$"} className="pyforge-text" mode="latex" />
            );
            getByText(/latex/i);
            await waitFor(() => expect(container.querySelector(".pyforge-text-latex")).toBeInTheDocument());
        });
        it("renders LaTeX as inline math", async () => {
            const { container, getByText, findByText } = render(
                <Field value={"This is inline $x = y + 1$ math."} className="pyforge-text" mode="latex" />
            );
