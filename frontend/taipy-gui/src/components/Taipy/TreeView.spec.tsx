...
describe("TreeView Component", () => {
    ...
    it("uses the class", async () => {
        const { getByText } = render(<TreeView lov={lov} className="pyforge-tree" />);
        const elt = getByText("Item 1");
        expect(elt.parentElement?.parentElement?.parentElement?.parentElement?.parentElement).toHaveClass("pyforge-tree");
    });
    ...
    it("does not dispatch update message when expanded is boolean", async () => {
        const dispatch = jest.fn();
        const state: PyForgeState = INITIAL_STATE;
        const { getByText } = render(<PyForgeContext.Provider value={{ state, dispatch }}><TreeView lov={lov} expanded={true} updateVars="expanded=tree_expanded" /></PyForgeContext.Provider>);
        const elt = getByText("Item 1");
        await userEvent.click(elt);
    ...
    it("does dispatch update message when expanded is not boolean", async () => {
        const dispatch = jest.fn();
        const state: PyForgeState = INITIAL_STATE;
        const { getByText } = render(<PyForgeContext.Provider value={{ state, dispatch }}><TreeView lov={lov} expanded={[]} updateVars="expanded=tree_expanded" /></PyForgeContext.Provider>);
        const elt = getByText("Item 1");
        const icon = elt.parentElement?.querySelector(".MuiTreeItem-iconContainer") || elt;
        await userEvent.click(icon);
