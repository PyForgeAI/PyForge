    ...
    "0--1-bool,int,float,Code-asc": {
        data: [
            ...
            },
            {
                ...
                int: 478,
                float: 3.5,
                Code: "![PyForge!](https://docs.pyforge.io/en/latest/assets/images/favicon.png)",
            },
        ],
        rowcount: 2,
        start: 0,
...
describe("PaginatedTable Component", () => {
    ...
    it("displays the right info for class", async () => {
        const { getByText } = render(
            <PaginatedTable data={undefined} defaultColumns={tableColumns} className="pyforge-table" />
        );
        const elt = getByText("Entity").closest("table");
        expect(elt?.parentElement?.parentElement?.parentElement).toHaveClass("pyforge-table", "pyforge-table-paginated");
    });
    ...
    it("dispatch a well formed message on page change", async () => {
        const dispatch = jest.fn();
        const state: PyForgeState = { ...INITIAL_STATE, data: { table: undefined } };
        ...
        rerender(
            <PyForgeContext.Provider value={{ state: newState, dispatch }}>
                <PaginatedTable
                    id="table"
    ...
    it("selects the rows", async () => {
        ...
        rerender(
            <PyForgeContext.Provider value={{ state: { ...state }, dispatch }}>
    ...
    it("can show an image", async () => {
        ...
        dispatch.mockClear();
        const elt = getByAltText("PyForge!");
        ...
        dispatch.mockClear();
        const elt2 = getByAltText("PyForge!");
