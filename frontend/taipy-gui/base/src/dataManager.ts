...
export class DataManager {
    ...
    // Initial data fetched from pyforge-gui backend
    ...
    get(encodedName: string, dataEventKey?: string): unknown {
        ...
        if (dataEventKey) {
            if (!(encodedName in this._requested_data)) {
                throw new Error(`Encoded name '${encodedName}' is not available in PyForge GUI`);
            }
            if (!(dataEventKey in this._requested_data[encodedName])) {
                throw new Error(`Event key '${dataEventKey}' is not available for encoded name '${encodedName}' in PyForge GUI`);
            }
            return this._requested_data[encodedName][dataEventKey].receivedData;
        }
        // handle normal data
        if (!(encodedName in this._data)) {
            throw new Error(`${encodedName} is not available in PyForge GUI`);
        }
