...
import { PyForgeWsAdapter, WsAdapter } from "./wsAdapter";
import { WsMessageType } from "../../src/context/wsUtils";
import { getBase } from "./utils";
import { CookieHandler } from "./cookieHandler";

export type OnInitHandler = (pyforgeApp: PyForgeApp) => void;
export type OnChangeHandler = (pyforgeApp: PyForgeApp, encodedName: string, value: unknown, dataEventKey?: string) => void;
export type OnNotifyHandler = (pyforgeApp: PyForgeApp, type: string, message: string) => void;
export type OnReloadHandler = (pyforgeApp: PyForgeApp, removedChanges: ModuleData) => void;
export type OnWsMessage = (pyforgeApp: PyForgeApp, event: string, payload: unknown) => void;
export type OnWsStatusUpdate = (pyforgeApp: PyForgeApp, messageQueue: string[]) => void;
export type OnEvent =
    | OnInitHandler
    | OnChangeHandler
    | OnNotifyHandler
...
type RequestDataCallback = (pyforgeApp: PyForgeApp, encodedName: string, dataEventKey: string, value: unknown) => void;

export class PyForgeApp {
    socket: Socket;
    ...
    constructor(
        ...
        this.path = path;
        this.socket = socket;
        this.wsAdapters = [new PyForgeWsAdapter()];
        this._ackList = [];
        this._rdc = {};
    ...
    // This update will only send the request to PyForge Gui backend
    ...
    // Request Data from pyforge backend
...
export const createApp = (
    ...
    return new PyForgeApp(onInit, onChange, path, socket, handleCookie);
};
