...
def hash_files_in_frontend_folder(frontend_folder):
    ...
    file_hashes = {}
    lookup_fe_folder = [f"{frontend_folder}{os.sep}pyforge", f"{frontend_folder}{os.sep}pyforge-gui"]
    if len(sys.argv) > 1:
        if sys.argv[1] == "--pyforge-gui-only":
            lookup_fe_folder.pop(0)
        elif sys.argv[1] == "--pyforge-gui-core-only":
...
if __name__ == "__main__":
    ...
    print(f"PyForge Frontend hash {combined_hash}")  # noqa: T201
