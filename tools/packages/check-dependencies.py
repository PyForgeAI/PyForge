...
class Package:
    ...
    # PyForge dependencies are ignored.
    is_pyforge: bool
    ...
    def check_format(cls, package: str):
        ...
        if ">=" not in package or "<" not in package:
            # Only PyForge dependencies can be without version.
            if "pyforge" not in package:
    ...
    def from_requirements(cls, package: str, filename: str):
        ...
        try:
            ...
            is_pyforge = "pyforge" in name
            return cls(
                name,
                extract_min_version(package) if not is_pyforge else "",
                extract_max_version(package) if not is_pyforge else "",
                extract_installation_markers(package) if not is_pyforge else "",
                is_pyforge,
...
if __name__ == "__main__":
    if sys.argv[1] == "ensure-same-version":
        ...
        _requirements_filenames = glob.glob("pyforge*/*requirements.txt")
