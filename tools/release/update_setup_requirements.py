...
def __build_pyforge_package_line(line: str, version: str, publish_on_py_pi: bool) -> str:
    _line = line.strip()
    if publish_on_py_pi:
        return f"{_line}=={version}\n"
    tag = f"{version}-{_line.split('-')[1]}"
    tar_name = f"{_line}-{version}"
    return f"{_line} @ https://github.com/Avaiga/pyforge/releases/download/{tag}/{tar_name}.tar.gz\n"
    ...
    with open(_path, mode="r") as req:
        for line in req:
            if v := versions.get(line.strip()):
                line = __build_pyforge_package_line(line, v, publish_on_py_pi)
            lines.append(line)
...
if __name__ == "__main__":
    _package = sys.argv[1]
    _versions = {
        "pyforge-common": sys.argv[2],
        "pyforge-core": sys.argv[3],
        "pyforge-gui": sys.argv[4],
        "pyforge-rest": sys.argv[5],
        "pyforge-templates": sys.argv[6],
    }
