...
def get_requirements(pkg: str, env: str = "dev") -> list:
    # get requirements from the different setups in tools/packages (removing pyforge packages)
    reqs = set()
    pkg_name = pkg if pkg == "pyforge" else f"pyforge-{pkg}"
    ...
    if env == "dev":
        return [r for r in reqs if r and not r.startswith("pyforge")]
    return list(reqs)
...
def update_pyproject(version_path: str, pyproject_path: str, env: str = "dev"):
    ...
    pyproject_data["project"]["urls"]["Release notes"] = f"https://docs.pyforge.io/en/release-{version_string}/relnotes/"
...
def _build_webapp(webapp_path: str):
    ...
    if not already_exists:
        os.system("cd ../../frontend/pyforge-gui/dom && npm ci")
        os.system("cd ../../frontend/pyforge-gui && npm ci && npm run build")
...
if __name__ == "__main__":
    ...
    if pkg == "pyforge":
        _version_path = os.path.join(sys.argv[1], "pyforge", "version.json")
        _webapp_path = os.path.join(sys.argv[1], "pyforge", "gui", "webapp", "index.html")
    else:
