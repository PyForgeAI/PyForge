...
def __migrate_subscriber(fct_module, fct_name):
    ...
    if fct_module == "pyforge.core._scheduler._scheduler":
...
def __migrate_global_config(config: Dict):
    ...
    if "core_version" not in config["CORE"] or config["CORE"]["core_version"] != version("pyforge-core"):
        config["CORE"]["core_version"] = version("pyforge-core")
