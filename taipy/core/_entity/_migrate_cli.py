...
class _MigrateCLI(_AbstractCLI):
    ...
    def create_parser(cls):
        migrate_parser = _PyForgeParser._add_subparser(
            cls._COMMAND_NAME,
            help="Migrate entities created from old pyforge versions to be compatible with the current pyforge version. "
            " The entity migration should be performed only after updating pyforge code to the current version.",
        )
    ...
    def handle_command(cls):
        ...
        if not args.repository_type:
            _PyForgeParser._sub_pyforgeparsers.get(cls._COMMAND_NAME).print_help()
    ...
    def __handle_remove_backup(cls, repository_type: str, repository_args: List):
        if repository_type == "filesystem":
            path = repository_args[0] or Config.core.pyforge_storage_folder
