...
def _check_dependency_is_installed(module_name: str, package_name: str) -> None:
    """
        ...
        Args:
            module_name: Name of the pyforge module importing the package.
    ...
    if not util.find_spec(package_name):
        raise RuntimeError(
            ...
            f"using `pip install pyforge[{extras.get(package_name)}]`."
        )


class EnterpriseEditionUtils:
    _TAIPY_ENTERPRISE_MODULE = "pyforge.enterprise"
