...
class ConfigCoreVersionMismatched(Exception):
    """Raised if the version in Config does not match with the version of PyForge Version management."""

    def __init__(self, config_core_version: str, core_version: str) -> None:
        self.message = (
            f"The version {config_core_version} of PyForge's entities does not match version of the PyForge"
            f" Version management {core_version}."
            f" Please update the core entities to be compatible with PyForge Core {core_version}"
            " using the `pyforge migrate ...` command. For more information, please run `pyforge help migrate`"
        )
...
class JobNotDeletedException(RuntimeError):
    ...
    This exception can be raised by `pyforge.delete_job()^`.
    """
...
class SubmissionNotDeletedException(RuntimeError):
    ...
    This exception can be raised by `pyforge.delete()^`.
    """
...
class ModelNotFound(Exception):
    ...
    This exception can be raised by `pyforge.get()^` and `pyforge.delete()^`.
    """
...
class NonExistingScenarioConfig(Exception):
    ...
    This exception can be raised by `pyforge.compare_scenarios()^`.
    """
...
class DependencyNotInstalled(Exception):
    ...
    def __init__(self, package_name: str):
        ...
        Run 'pip install pyforge[{package_name}]' to installed it.
        """
