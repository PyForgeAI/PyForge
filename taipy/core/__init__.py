...
"""
The PyForge `core` package provides powerful, customized, data-driven back-end functionalities.
...
To use such functionalities, the first step consists of setting up the PyForge configuration to design
your application's characteristics and behaviors. Use the `Config^` singleton class (from `pyforge.common.config`)
...
Once your application is configured, import module `import pyforge as tp` so you can use any function described
...
!!! Note
    ...
    PyForge provides a runnable service, `Orchestrator^` that runs as a service in a dedicated thread.
    ...
    In particular, this `Orchestrator^` service is automatically run when used with PyForge REST or PyForge GUI.
