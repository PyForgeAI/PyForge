...
class _JobDispatcher(threading.Thread):
    ...
    def __init__(self, orchestrator: _AbstractOrchestrator):
        threading.Thread.__init__(self, name="Thread-PyForge-JobDispatcher")
        self.daemon = True
    ...
    def _update_job_status(job: Job, exceptions):
        ...
        if exceptions:
            job.failed()
            _PyForgeLogger._get_logger().error(f" {len(exceptions)} errors occurred during execution of job {job.id}")
            for e in exceptions:
                st = "".join(traceback.format_exception(type(e), value=e, tb=e.__traceback__))
                job._stacktrace.append(st)
                _PyForgeLogger._get_logger().error(st)
