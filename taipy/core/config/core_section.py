...
from pyforge.common.config import Config, UniqueSection
from pyforge.common.config._config import _Config
from pyforge.common.config.common._config_blocker import _ConfigBlocker
from pyforge.common.config.common._template_handler import _TemplateHandler as _tpl
from pyforge.core._version._version_manager_factory import _VersionManagerFactory
from pyforge.core.cycle._cycle_manager_factory import _CycleManagerFactory
from pyforge.core.data._data_manager_factory import _DataManagerFactory
from pyforge.core.job._job_manager_factory import _JobManagerFactory
from pyforge.core.scenario._scenario_manager_factory import _ScenarioManagerFactory
from pyforge.core.sequence._sequence_manager_factory import _SequenceManagerFactory
from pyforge.core.submission._submission_manager_factory import _SubmissionManagerFactory
from pyforge.core.task._task_manager_factory import _TaskManagerFactory
...
class CoreSection(UniqueSection):
    ...
    _DEFAULT_ROOT_FOLDER = "./pyforge/"

    _STORAGE_FOLDER_KEY = "storage_folder"
    _DEFAULT_STORAGE_FOLDER = "user_data/"

    _STORAGE_FOLDER_TP_KEY = "pyforge_storage_folder"
    _DEFAULT_STORAGE_FOLDER_TP = ".pyforge/"
    ...
    def __init__(
        ...
        pyforge_storage_folder: Optional[str] = None,
        ...
        **properties,
        ...
        self._pyforge_storage_folder = pyforge_storage_folder
    ...
    def root_folder(self) -> str:
        """ Path of the base folder for the pyforge application.

        The default value is "./pyforge/".
        """
    ...
    def storage_folder(self) -> str:
        ...
        <root_folder><storage_folder> (The default path is "./pyforge/user_data/").
        """
    ...
    @property
    def pyforge_storage_folder(self) -> str:
        """Folder name used to store internal PyForge data.

        The default value is ".pyforge/".
        """
        return _tpl._replace_templates(self._pyforge_storage_folder)

    @pyforge_storage_folder.setter  # type: ignore
    @_ConfigBlocker._check()
    def pyforge_storage_folder(self, val) -> None:
        self._pyforge_storage_folder = val

    @property
    def repository_type(self) -> str:
        """Type of the repository to be used to store PyForge data.
    ...
    @property
    def mode(self) -> str:
        """The operating mode of PyForge.

        PyForge applications are versioned. The versioning system is used to manage
        the different versions of the user application. Depending on the
        operating mode, PyForge will behave differently when a version of the
        ...
        By default, PyForge runs in "development" mode.
        """
    ...
    def force(self) -> bool:
        ...
        the configuration of the last run application, PyForge will exit. If the *force*
        attribute is set to True, PyForge will run even if there are some conflicts.
        """
    ...
    def core_version(self) -> str:
        """The version of the PyForge core library."""
    ...
    def _clean(self):
        ...
        self._pyforge_storage_folder = self._DEFAULT_STORAGE_FOLDER
    ...
    def _to_dict(self):
        ...
        if self._pyforge_storage_folder:
            as_dict[self._STORAGE_FOLDER_TP_KEY] = self._pyforge_storage_folder
    ...
    def _from_dict(cls, as_dict: Dict[str, Any], id=None, config: Optional[_Config] = None):
        ...
        pyforge_storage_folder = as_dict.pop(cls._STORAGE_FOLDER_TP_KEY, None)
    ...
    def _update(self, as_dict: Dict[str, Any], default_section=None):
        ...
        self._pyforge_storage_folder = as_dict.pop(self._STORAGE_FOLDER_TP_KEY, self._pyforge_storage_folder)
    ...
    def _configure(
        ...
        **properties,
        ...
        Arguments:
            root_folder (Optional[str]): Path of the base folder for the pyforge application.
                The default value is "./pyforge/"
            storage_folder (str): Folder name used to store user data. The default value is "user_data/". It is used in
                ...
                <root_folder><storage_folder> (The default path is "./pyforge/user_data/").
            pyforge_storage_folder (str): Folder name used to store PyForge data. The default value is ".pyforge/". It is
                used in conjunction with the *root_folder* attribute. That means the storage path is
                <root_folder><storage_folder> (The default path is "./pyforge/.pyforge/").
            repository_type (Optional[str]): The type of the repository to be used to store PyForge data.
                The default value is "filesystem".
            ...
            mode (Optional[str]): Indicates the mode of the version management system.
                Possible values are *"development"* or *"experiment"*. On Enterprise edition of PyForge,
            ...
            force (Optional[bool]): If True, PyForge will override a version even if the configuration
        ...
        section = CoreSection(
            ...
            pyforge_storage_folder=pyforge_storage_folder,
