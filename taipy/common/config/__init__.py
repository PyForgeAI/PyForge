...
"""The `pyforge.common.config` package provides features to configure a PyForge application.

Its main class is the `Config^` singleton. It exposes various static methods
and attributes to configure the PyForge application and retrieve the configuration values.

!!! example "Standard usage"

    ```python
    from pyforge.common.config import Config
    from pyforge.common.config import Frequency
    from pyforge.common.config import Scope
    ...
    Config.configure_authentication(protocol="pyforge",roles={"user1": ["role1", "TAIPY_READER"]})
    ...
    a PyForge application. The application has one data node configuration and one scenario
    configuration.
...
!!! note "`Frequency^` and `Scope^` for scenario and data nodes configurations"
    ...
    Besides the `Config^` class which is the main entrypoint, the `pyforge.common.config` package exposes
    the `Frequency^` and `Scope^` enums that are frequently used to configure data nodes and
    scenarios.

    The three objects are exposed in the `pyforge^` package directly for convenience.
