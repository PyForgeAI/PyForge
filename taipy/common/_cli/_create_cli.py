...
from ._base_cli._pyforge_parser import _PyForgeParser
...
class _CreateCLI(_AbstractCLI):
    ...
    def generate_template_map(cls):
        template_path = pathlib.Path(pyforge.__file__).parent.resolve() / "templates"
    ...
    def create_parser(cls):
        create_parser = _PyForgeParser._add_subparser(
            cls._COMMAND_NAME,
            help="Create a new PyForge application using pre-defined templates.",
        )
        create_parser.add_argument(
            "--application",
            choices=list(cls._template_map.keys()),
            default="default",
            help="The template used to create the new PyForge application.",
        )
