...
class _RunCLI(_AbstractCLI):
    ...
    def create_parser(cls):
        run_parser = _PyForgeParser._add_subparser(cls._COMMAND_NAME, help="Run a PyForge application.")
    ...
    def handle_command(cls):
        ...
        pyforge_args = [f"--pyforge-{arg[2:]}" if arg.startswith("--") else arg for arg in all_args]

        try:
            subprocess.run(
                [sys.executable, args.application_main_file, *(external_args + pyforge_args)],
                stdout=sys.stdout,
                stderr=sys.stdout,
            )
