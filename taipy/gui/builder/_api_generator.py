...
class _ElementApiGenerator(object, metaclass=_Singleton):
    ...
    def add_default(self):
        ...
        if current_frame.f_back is None:
            raise RuntimeError(f"{error_message}: pyforge-gui module not found.")
    ...
    def add_library(self, library: "ElementLibrary"):
        ...
        if self.__module is None:
            _PyForgeLogger._get_logger().info(
                f"Python API for extension library '{library_name}' is not available. To fix this, import 'pyforge.gui.builder' before importing the extension library."  # noqa: E501
            )
            return
        ...
        for element_name, element in library.get_elements().items():
            ...
            # Allow element to be accessed from this module (pyforge.gui.builder)
