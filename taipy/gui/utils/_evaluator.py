...
class _Evaluator:
    ...
    def __init__(self, default_bindings: t.Dict[str, t.Any], shared_variable: t.List[str]) -> None:
        ...
        # expr to holders
        self.__expr_to_holders: t.Dict[str, t.Set[t.Type[_PyForgeBase]]] = {}
                ...
                if isinstance(node, ast.Call):
                    ...
                    if var_name in builtin_vars:
                        if node not in functionsCalls:
                            _warn(
                                f"Variable '{var_name}' cannot be used in PyForge expressions "
                        ...
                        try:
                            if lazy_declare and var_name.startswith("__"):
                                with warnings.catch_warnings(record=True) as warns:
                                    ...
                                    if next((w for w in warns if w.category is PyForgeGuiWarning), None):
    ...
    def evaluate_bind_holder(self, gui: Gui, holder: t.Type[_PyForgeBase], expr: str) -> str:
    ...
    @staticmethod
    def __get_holder_hash(holder: t.Type[_PyForgeBase], expr_hash: str) -> str:
        return f"{holder.get_hash()}_{_get_client_var_name(expr_hash)}"

    def __evaluate_holder(self, gui: Gui, holder: t.Type[_PyForgeBase], expr: str) -> t.Optional[_PyForgeBase]:
        expr_hash = ""
        try:
            expr_hash = self.__expr_to_hash.get(expr, "unknownExpr")
            ...
            if not isinstance(holder_value, _PyForgeBase):
    ...
    def refresh_expr(self, gui: Gui, var_name: str, holder: t.Optional[_PyForgeBase]):
        """
    ...
    def re_evaluate_expr(self, gui: Gui, var_name: str) -> t.Set[str]:  # noqa C901
        ...
        for expr in self.__var_to_expr_list[var_name]:
            ...
            if expr != var_name and not expr.startswith(_PyForgeBase._HOLDER_PREFIX):
