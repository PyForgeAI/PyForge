...
from ._renderers.json import _PyForgeJsonProvider
...
class _Server:
    ...
    def __init__(
        ...
        if self._flask is None:
            flask_config: t.Dict[str, t.Any] = {"import_name": "PyForge"}
        ...
        # set json encoder (for PyForge specific types)
        self._flask.json_provider_class = _PyForgeJsonProvider
        ...
        def handle_message(message, *args) -> None:
            if "status" in message:
                _PyForgeLogger._get_logger().info(message["status"])
    ...
    def _get_default_blueprint(
        ...
        pyforge_bp = Blueprint("PyForge", __name__, static_folder=static_folder, template_folder=template_folder)
        # Serve static react build

        @pyforge_bp.route("/", defaults={"path": ""})
        @pyforge_bp.route("/<path:path>")
        def my_index(path):
            ...
            if path == "" or path == "index.html" or "." not in path:
                try:
                    ...
                    raise RuntimeError(
                        "Something is wrong with the pyforge-gui front-end installation. Check that the js bundle has been properly built (is Node.js installed?)."  # noqa: E501
                    ) from None

            if path == "pyforge.status.json":
    ...
    def run(
        ...
        if debug and not is_running_from_reloader() and _is_port_open(host_value, port):
            raise ConnectionError(
                f"Port {port} is already opened on {host} because another application is running on the same port.\nPlease pick another port number and rerun with the 'port=<new_port>' setting.\nYou can also let PyForge choose a port number for you by running with the 'port=\"auto\"' setting."  # noqa: E501
            )
        if not flask_log:
            log = logging.getLogger("werkzeug")
            log.disabled = True
            if not is_running_from_reloader():
                _PyForgeLogger._get_logger().info(f" * Server starting on {server_url}")
            else:
                _PyForgeLogger._get_logger().info(f" * Server reloaded on {server_url}")
            if client_url is not None:
                client_url = client_url.format(port=port)
                _PyForgeLogger._get_logger().info(f" * Application is accessible at {client_url}")
