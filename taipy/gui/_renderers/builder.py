...
from ..types import PropertyType, _get_pyforge_type
from ..utils import (
    _date_to_string,
...
from ..utils.types import _PyForgeBase, _PyForgeData, _PyForgeToJson
from .json import _PyForgeJsonEncoder
...
class _Builder:
    ...
    This class can only be instantiated internally by PyForge.
    """
    ...
    def __set_json_attribute(self, name, value):
        return self.set_attribute(name, json.dumps(value, cls=_PyForgeJsonEncoder))
    ...
    def _get_dataframe_attributes(self) -> "_Builder":
        ...
        cols_description = self.__gui._get_accessor().get_cols_description(data_hash, _PyForgeData(data, data_hash))
    ...
    def _get_chart_config(self, default_type: str, default_mode: str):
        ...
        if data_hash:
            ...
            while add_data_hash := self.__hashes.get(name_idx):
                typed_hash = self.__get_typed_hash_name(add_data_hash, _PyForgeData)
                ...
                cols_description.append(
                    self.__gui._get_accessor().get_cols_description(add_data_hash, _PyForgeData(add_data, add_data_hash))
                )
    ...
    def __set_class_names(self):
        ...
        if (private_css := self.__prop_values.get("style")) and isinstance(private_css, (dict, _MapDict)):
            pyforge_style = etree.Element("PyForgeStyle")
            pyforge_style.set("className", f"tpcss-{id(private_css)}")
            pyforge_style.set(
                "content",
                json.dumps(private_css if isinstance(private_css, dict) else private_css._dict),
            )
            pyforge_style.text = "style"
            self.el.append(pyforge_style)
    ...
    def __set_default_value(
        ...
        var_type: t.Optional[t.Union[PropertyType, t.Type[_PyForgeBase]]] = None,
    ):
        if value is None:
            value = self.__prop_values.get(var_name)
        ...
        if isinstance(value, (datetime, date, time)):
            ...
            return self
        elif isclass(var_type) and issubclass(var_type, _PyForgeBase):
    ...
    def __get_typed_hash_name(
        self, hash_name: str, var_type: t.Optional[t.Union[PropertyType, t.Type[_PyForgeBase]]]
    ) -> str:
        if pyforge_type := _get_pyforge_type(var_type):
            expr = self.__gui._get_expr_from_hash(hash_name)
            hash_name = self.__gui._evaluate_bind_holder(t.cast(t.Type[_PyForgeBase], pyforge_type), expr)
        return hash_name
    ...
    def _build_to_string(self):
        ...
        el_str = el_str.replace(">style</PyForgeStyle>", "/>")
