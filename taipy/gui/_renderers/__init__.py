...
from ._html import _PyForgeHTMLParser
    ...
    def __parse_file_content(self, content):
        with open(t.cast(str, content), "rb") as f:
            ...
            if self._encoding is not None:
                ...
                _PyForgeLogger._get_logger().info(f"'{encoding}' encoding was used to decode file '{content}'.")
            elif (detected_encoding := t.cast(str, detect(file_content).get("encoding"))) is not None:
                encoding = detected_encoding
                _PyForgeLogger._get_logger().info(f"Detected '{encoding}' encoding for file '{content}'.")
            else:
                _PyForgeLogger._get_logger().info(f"Using default '{encoding}' encoding for file '{content}'.")
...
class Markdown(_Renderer):
    ...
    PyForge can use Markdown text to create pages that are the base of
    user interfaces.

    You can find details on the PyForge Markdown-specific syntax and how to add
    PyForge Visual Elements in the [section on Markdown](../../../../../userman/gui/pages/markdown.md)
    of the User Manual.
    """
    ...
    def __init__(self, content: str, **kwargs) -> None:
        ...
        `(pyforge.gui.Page.)set_style()^` method.
        """
...
class Html(_Renderer):
    ...
    PyForge can use HTML code to create pages that are the base of
    user interfaces.

    You can find details on HTML-specific constructs and how to add
    PyForge Visual Elements in the [section on HTML](../../../../../userman/gui/pages/html.md)
    of the User Manual.
    """
    ...
    # Modify path routes
    def modify_pyforge_base_url(self, base_url):
        self._content = self._content.replace("{{pyforge_base_url}}", f"{base_url}")

    # Generate JSX from HTML
    def render(self, gui: "Gui") -> str:
        parser = _PyForgeHTMLParser(gui)
