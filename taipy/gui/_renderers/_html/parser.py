...
class _PyForgeHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag, props) -> None:
        ...
        if self.head_tag is not None:
            ...
            self.head_tag = None
        if self.pyforge_tag is not None:
            self.parse_pyforge_tag()
        if tag == "head":
            self.is_body = False
        elif tag == "body":
            self.is_body = True
        elif m := self.__TAIPY_NAMESPACE_RE.match(tag):
            self.pyforge_tag = _PyForgeTag(m.group(1), m.group(2), props)
    ...
    def handle_data(self, data: str) -> None:
        data = data.strip()
        if data and self.pyforge_tag is not None and self.pyforge_tag.set_value(data):
            self.parse_pyforge_tag()
    ...
    def parse_pyforge_tag(self) -> None:
        tp_string, tp_element_name = self.pyforge_tag.parse(self._gui)
        self.append_data(tp_string)
        self.tag_mapping[f"{self.pyforge_tag.namespace}:{self.pyforge_tag.control_type}"] = tp_element_name
        self.pyforge_tag = None
...
class _PyForgeTag(object):
    ...
    def parse(self, gui) -> t.Tuple[str, str]:
        ...
        # allow usage of 'class' property in html pyforge tag
