...
from ._renderers._markdown import _PyForgeMarkdownExtension
from ._renderers.factory import _Factory
from ._renderers.json import _PyForgeJsonEncoder
from ._renderers.utils import _get_columns_dict
from ._warnings import PyForgeGuiWarning, _warn
...
class Gui:
    ...
    !!! note
        This class belongs to and is documented in the `pyforge.gui` package, but it is
        accessible from the top `pyforge` package to simplify its access, allowing to
        use:
        ```py
        from pyforge import Gui
        ```
    """

    __root_page_name = "TaiPy_root_page"
    __env_filename = "pyforge.gui.env"
    __UI_BLOCK_NAME = "PyForgeUiBlockVar"
    __MESSAGE_GROUPING_NAME = "PyForgeMessageGrouping"
    __ON_INIT_NAME = "PyForgeOnInit"
    __ARG_CLIENT_ID = "client_id"
    __INIT_URL = "pyforge-init"
    __JSX_URL = "pyforge-jsx"
    __CONTENT_ROOT = "pyforge-content"
    __UPLOAD_URL = "pyforge-uploads"
    _EXTENSION_ROOT = "pyforge-extension"
    __USER_CONTENT_URL = "pyforge-user-content"
    __BROADCAST_G_ID = "pyforge_broadcasting"
    __BRDCST_CALLBACK_G_ID = "pyforge_brdcst_callback"
    __SELF_VAR = "__gui"
    __DO_NOT_UPDATE_VALUE = _DoNotUpdate()
    _HTML_CONTENT_KEY = "__pyforge_html_content"
    ...
    __DOWNLOAD_ACTION = "__PyForge__download_csv"
    __DOWNLOAD_DELETE_ACTION = "__PyForge__download_delete_csv"
    __DEFAULT_FAVICON_URL = "https://raw.githubusercontent.com/Avaiga/pyforge-assets/develop/favicon.png"
    ...
    def __init__(
        ...
        Arguments:
            ...
            env_filename (Optional[str]): An optional file from which to load application
                ...
                The default value is "pyforge.gui.env"
        ...
        self._markdown = md_lib.Markdown(
            extensions=[
                ...
                "attr_list",
                "md_in_html",
                _PyForgeMarkdownExtension(gui=self),
            ]
        )
    ...
    def __process_content_provider(self, state: State, path: str, query: t.Dict[str, str]):
        ...
        if variable_name:
            ...
            if isinstance(content, _PyForgeContentHtml):
                content = content.get()
    ...
    def __front_end_update(
        ...
        if isinstance(current_value, _PyForgeData):
            return
        elif rel_var and isinstance(current_value, _PyForgeLovValue):  # pragma: no cover
            lov_holder = _getscopeattr_drill(self, self.__evaluator.get_hash_from_expr(rel_var))
            if isinstance(lov_holder, _PyForgeLov):
                if isinstance(value, (str, list)):
                    ...
                    if isinstance(value, list):
                        ...
                        value = ret_val[0]
        elif isinstance(current_value, _PyForgeBase):
            value = current_value.cast_value(value)
        self._update_var(
            var_name, value, propagate, current_value if isinstance(current_value, _PyForgeBase) else None, on_change
        )
    ...
    def _update_var(
        ...
        value: t.Any,
        propagate=True,
        holder: t.Optional[_PyForgeBase] = None,
        on_change: t.Optional[str] = None,
        ...
        if forward:
            ...
            if not var_modified:
                self._call_on_change(
                    var_name,
                    value.get()
                    if isinstance(value, _PyForgeBase)
                    else value._dict
    ...
    def _get_real_var_name(self, var_name: str) -> t.Tuple[str, str]:
        ...
        if var_name.startswith(_PyForgeBase._HOLDER_PREFIX):
            for hp in _PyForgeBase._get_holder_prefixes():
    ...
    def __send_var_list_update(  # noqa C901
        ...
        for k, v in values.items():
            if isinstance(v, (_PyForgeData, _PyForgeContentHtml)) and v.get_name() in modified_vars:
        ...
        for _var in modified_vars:
            ...
            if isinstance(newvalue, (_PyForgeData)) or isinstance(newvalue, custom_page_filtered_types):
                newvalue = {"__pyforge_refresh": True}
            else:
                if isinstance(newvalue, (_PyForgeContent, _PyForgeContentImage)):
                    ret_value = self.__get_content_accessor().get_info(
                        t.cast(str, front_var), newvalue.get(), isinstance(newvalue, _PyForgeContentImage)
                    )
                    if isinstance(ret_value, tuple):
                        newvalue = f"/{Gui.__CONTENT_ROOT}/{ret_value[0]}"
                    else:
                        newvalue = ret_value
                elif isinstance(newvalue, _PyForgeContentHtml):
                    newvalue = self._get_user_content_url(
                        None, {"variable_name": str(_var), Gui._HTML_CONTENT_KEY: str(time.time())}
                    )
                elif isinstance(newvalue, (_PyForgeLov, _PyForgeLovValue)):
                    newvalue = self.__adapter.run(
                        newvalue.get_name(), newvalue.get(), id_only=isinstance(newvalue, _PyForgeLovValue)
                    )
                elif isinstance(newvalue, _PyForgeBase):
                    newvalue = newvalue.get()
                # Skip in pyforge-gui, available in custom frontend
                ...
                if newvalue is not None and not isinstance(newvalue, str):
                    ...
                    with warnings.catch_warnings(record=True) as warns:
                        ...
                        json.dumps(newvalue, cls=_PyForgeJsonEncoder)
                        if len(warns):
                            keep_value = True
                            for w in warns:
                                if is_debugging():
    ...
    def set_unsupported_data_converter(converter: t.Optional[t.Callable[[t.Any], t.Any]]) -> None:
        ...
        be serialized automatically. When PyForge GUI encounters an unsupported type in the data
    ...
    def __request_data_update(self, var_name: str, payload: t.Any) -> None:
        ...
        resource_handler = get_current_resource_handler()
        custom_page_filtered_types = resource_handler.data_layer_supported_types if resource_handler else ()
        if not isinstance(newvalue, _PyForgeData) and isinstance(newvalue, custom_page_filtered_types):
            newvalue = _PyForgeData(newvalue, "")
        if isinstance(newvalue, _PyForgeData):
            ret_payload = None
        ...
        if isinstance(payload, dict) and isinstance(payload.get("names"), list):
            ...
            if payload.get("refresh", False):
                ...
                for _var in t.cast(list, payload.get("names")):
                    ...
                    self._refresh_expr(
                        val.get_name() if isinstance(val, _PyForgeBase) else _var,
                        val if isinstance(val, _PyForgeBase) else None,
                    )
    ...
    def __handle_ws_get_module_context(self, payload: t.Any):
        if isinstance(payload, dict):
            ...
            if mc := self._get_page_context(page_path):
                ...
                meta_return = json.dumps(metadata, cls=_PyForgeJsonEncoder) if metadata else None
                self.__send_ws(
                    {
    ...
    def _refresh_expr(self, var_name: str, holder: t.Optional[_PyForgeBase]):
    ...
    def _evaluate_bind_holder(self, holder: t.Type[_PyForgeBase], expr: str) -> str:
            ...
            try:
                ...
                if rebuild:
                    ...
                    col_dict = _get_columns_dict(
                        ...
                        self._get_accessor().get_cols_description(data_hash, _PyForgeData(data, data_hash)),
                        attributes.get("date_format"),
                        attributes.get("number_format"),
                    )
                    _enhance_columns(attributes, hashes, t.cast(dict, col_dict), "table(cols)")

                    return json.dumps(col_dict, cls=_PyForgeJsonEncoder)
            except Exception as e:  # pragma: no cover
                    ...
                    config = _build_chart_config(
                        ...
                        [
                            self._get_accessor().get_cols_description(
                                data_hash, _PyForgeData(kwargs.get(data_hash), data_hash)
                            )
                            for data_hash in data_hashes
                        ],
                    )

                    return json.dumps(config, cls=_PyForgeJsonEncoder)
            except Exception as e:  # pragma: no cover
    ...
    def __get_on_cancel_block_ui(self, callback: t.Optional[str]):
        def _pyforge_on_cancel_block_ui(a_state: State, id: t.Optional[str], payload: t.Any):
        ...
        return _pyforge_on_cancel_block_ui
    ...
    def __add_pages_in_folder(self, folder_name: str, folder_path: str):
        ...
        for file_name in list_of_files:
            ...
            if (re_match := Gui.__RE_HTML.match(file_name)) and f"{re_match.group(1)}.py" not in list_of_files:
                ...
                _renderers.modify_pyforge_base_url(folder_name)
    ...
    def add_pages(self, pages: t.Optional[t.Union[t.Mapping[str, t.Union[str, Page]], str]] = None) -> None:
        ...
        !!! note "Reading pages from a directory"
            ...
            this directory is traversed, recursively, to find files that PyForge can build
            pages from.
    ...
    def _set_css_file(self, css_file: t.Optional[str] = None):
        ...
        if css_file is None:
            ...
            elif script_file.is_dir() and (script_file / "pyforge.css").exists():
                css_file = "pyforge.css"
        if css_file is None:
            script_file = script_file.with_name("pyforge").with_suffix(".css")
    ...
    def __init_ngrok(self):
        ...
        if hasattr(self, "_ngrok"):
            ...
            if app_config.get("ngrok_token") == self._ngrok[1]:
                _PyForgeLogger._get_logger().info(f" * NGROK Public Url: {self._ngrok[0].public_url}")
                return
    ...
    def __register_blueprint(self):
        ...
        pages_bp = Blueprint("pyforge_pages", __name__)
        self._flask_blueprint.append(pages_bp)

        # server URL Rule for pyforge images
        images_bp = Blueprint("pyforge_images", __name__)
        ...
        upload_bp = Blueprint("pyforge_upload", __name__)
        ...
        user_content_bp = Blueprint("pyforge_user_content", __name__)
        ...
        extension_bp = Blueprint("pyforge_extensions", __name__)
        ...
        self._flask_blueprint.append(
            self._server._get_default_blueprint(
                ...
                title=self._get_config("title", "PyForge App"),
    ...
    def run(
        ...
        **kwargs,
        ...
        PyForgeGuiWarning.set_debug_mode(self._get_config("debug", False))
        ...
        # Base global ctx is PyForgeHolder classes + script modules and callables
        glob_ctx: t.Dict[str, t.Any] = {t.__name__: t for t in _PyForgeBase.__subclasses__()}
    ...
    def reload(self):  # pragma: no cover
        ...
        if hasattr(self, "_server") and hasattr(self._server, "_thread") and self._server._is_running:
            ...
            _PyForgeLogger._get_logger().info("Gui server has been reloaded.")
    ...
    def stop(self):
        ...
        if hasattr(self, "_server") and hasattr(self._server, "_thread") and self._server._is_running:
            self._server.stop_thread()
            _PyForgeLogger._get_logger().info("Gui server has been stopped.")
    ...
    def set_favicon(self, favicon_path: t.Union[str, Path], state: t.Optional[State] = None):
        ...
        pages) of PyForge GUI pages for a single or all connected clients.
        ...
        if state or self.__favicon != favicon_path:
            ...
            url = self._get_content("__pyforge_favicon", favicon_path, True)
            self._broadcast(
                "pyforge_favicon", url, self._get_client_id() if state else None, message_type=_WsType.FAVICON
            )
