...
class _Config(object):
    ...
    def _handle_argparse(self):
        ...
        config = self.config
        if args.pyforge_port:
            if str(args.pyforge_port).strip() == "auto":
                config["port"] = "auto"
            elif not _Config.__RE_PORT_NUMBER.match(args.pyforge_port):
                _warn("Port value for --port option is not valid.")
            else:
                config["port"] = int(args.pyforge_port)
        if args.pyforge_host:
            config["host"] = args.pyforge_host
        if args.pyforge_debug:
            config["debug"] = True
        if args.pyforge_no_debug:
            config["debug"] = False
        if args.pyforge_use_reloader:
            config["use_reloader"] = True
        if args.pyforge_no_reloader:
            config["use_reloader"] = False
        if args.pyforge_run_browser:
            config["run_browser"] = True
        if args.pyforge_no_run_browser:
            config["run_browser"] = False
        if args.pyforge_dark_mode or args.pyforge_light_mode:
            config["dark_mode"] = not args.pyforge_light_mode
        if args.pyforge_ngrok_token:
            config["ngrok_token"] = args.pyforge_ngrok_token
        if args.pyforge_webapp_path:
            config["webapp_path"] = args.pyforge_webapp_path
        elif os.environ.get("TAIPY_GUI_WEBAPP_PATH"):
            config["webapp_path"] = os.environ.get("TAIPY_GUI_WEBAPP_PATH")
        if args.pyforge_upload_folder:
            config["upload_folder"] = args.pyforge_upload_folder
        elif os.environ.get("TAIPY_GUI_UPLOAD_FOLDER"):
            config["webapp_path"] = os.environ.get("TAIPY_GUI_UPLOAD_FOLDER")
        if args.pyforge_client_url:
            config["client_url"] = args.pyforge_client_url
    ...
    def _build_config(self, root_dir, env_filename, kwargs):  # pragma: no cover
        ...
        # PyForge-config
        if find_spec("pyforge") and find_spec("pyforge.common.config"):
            from pyforge.common.config import Config as PyForgeConfig

            try:
                section = PyForgeConfig.unique_sections["gui"]
                self.config.update(section._to_dict())
            except KeyError:
                _warn("pyforge-common section for pyforge-gui is not initialized.")
