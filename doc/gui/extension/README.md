...
- `README.md`: This file.
- `main.py`: A Python script that can be used to run a PyForge GUI application that
...
the PyForge installation:

- If you build from a local copy (a clone, for example) of the
  [`pyforge` repository](https://github.com/Avaiga/pyforge/),
  this variable should be set to the path of the directory two levels above the directory where this
  README file is located, then down to the "pyforge" directory (i.e., the result of the Unix command
  "``readlink -f `pwd`/../../pyforge``").
- If you are building this extension library example from an installation of PyForge GUI, you can
  get that location issuing the command `pip show pyforge-gui`.

You can check that the setting is correct by verifying that the directory
"$TAIPY_DIR/pyforge/gui/webapp" exists.

A way to set this variable once and for all for your environment is to add the line:
```
TAIPY_DIR=<pyforge_installation_directory>
```
...
This example has all the important settings, ready to be built and used to run a PyForge GUI
...
- Extension Library directory: in this example, this is "example_library".<br/>
  ...
  - This is the name of the Python package to be imported by the PyForge GUI application
    script.<br/>
  ...
  - This directory name must appear in the paths that are used in the implementation
    ...
    PyForge GUI finds the JavaScript module to be loaded.
...
- Install the PyForge GUI JavaScript bundle and the other dependencies:<br/>
  You must run the command:
  ```bash
  npm install
  ```
  This command will fail with a message indicating that the PyForge GUI 'webapp' directory
...
creates a PyForge GUI application with a page that demonstrates the various
