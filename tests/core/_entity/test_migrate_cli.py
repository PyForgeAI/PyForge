...
def clean_data_folder():
    ...
    if os.path.exists("tests/core/_entity/.pyforge"):
        shutil.rmtree("tests/core/_entity/.pyforge")
    yield
...
def test_migrate_fs_default(caplog):
    ...
    assert "Starting entity migration from '.pyforge/' folder" in caplog.text


def test_migrate_fs_specified_folder(caplog, mocker):
    mocker.patch("pyforge.core._entity._migrate._utils.version", return_value="3.1.0")
    _MigrateCLI.create_parser()
...
@patch("pyforge.core._entity._migrate_cli._migrate_mongo_entities")
