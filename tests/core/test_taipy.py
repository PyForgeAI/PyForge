...
import pyforge.core.pyforge as tp
from pyforge.common.config import Config
from pyforge.common.config.exceptions.exceptions import ConfigurationUpdateBlocked
...
from pyforge.core._orchestrator._orchestrator_factory import _OrchestratorFactory
from pyforge.core._version._version_manager import _VersionManager
from pyforge.core.common.frequency import Frequency
from pyforge.core.common.scope import Scope
from pyforge.core.config.data_node_config import DataNodeConfig
from pyforge.core.config.scenario_config import ScenarioConfig
from pyforge.core.cycle._cycle_manager import _CycleManager
from pyforge.core.data._data_manager import _DataManager
from pyforge.core.data.pickle import PickleDataNode
from pyforge.core.exceptions.exceptions import DataNodeConfigIsNotGlobal
from pyforge.core.job._job_manager import _JobManager
from pyforge.core.job.job import Job
from pyforge.core.scenario._scenario_manager import _ScenarioManager
from pyforge.core.submission._submission_manager import _SubmissionManager
from pyforge.core.task._task_manager import _TaskManager
...
class TestPyForge:
    def test_set(self, scenario, cycle, sequence, data_node, task, submission):
        with mock.patch("pyforge.core.data._data_manager._DataManager._set") as mck:
            tp.set(data_node)
            mck.assert_called_once_with(data_node)
        with mock.patch("pyforge.core.task._task_manager._TaskManager._set") as mck:
            tp.set(task)
            mck.assert_called_once_with(task)
        with mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._set") as mck:
            tp.set(sequence)
            mck.assert_called_once_with(sequence)
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._set") as mck:
            tp.set(scenario)
            mck.assert_called_once_with(scenario)
        with mock.patch("pyforge.core.cycle._cycle_manager._CycleManager._set") as mck:
            tp.set(cycle)
            mck.assert_called_once_with(cycle)
        with mock.patch("pyforge.core.submission._submission_manager._SubmissionManager._set") as mck:
            tp.set(submission)
            mck.assert_called_once_with(submission)

    def test_is_editable_is_called(self, cycle, job, data_node):
        with mock.patch("pyforge.core.cycle._cycle_manager._CycleManager._is_editable") as mck:
        ...
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._is_editable") as mck:
        ...
        with mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._is_editable") as mck:
        ...
        with mock.patch("pyforge.core.task._task_manager._TaskManager._is_editable") as mck:
        ...
        with mock.patch("pyforge.core.job._job_manager._JobManager._is_editable") as mck:
            job_id = JobId("JOB_id")
        ...
        with mock.patch("pyforge.core.data._data_manager._DataManager._is_editable") as mck:
        ...
        with mock.patch("pyforge.core.submission._submission_manager._SubmissionManager._is_editable") as mck:
    ...
    def test_is_readable_is_called(self, cycle, job, data_node):
        with mock.patch("pyforge.core.cycle._cycle_manager._CycleManager._is_readable") as mck:
            cycle_id = CycleId("CYCLE_id")
        ...
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._is_readable") as mck:
            scenario_id = ScenarioId("SCENARIO_id")
        ...
        with mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._is_readable") as mck:
            sequence_id = SequenceId("SEQUENCE_id")
        ...
        with mock.patch("pyforge.core.task._task_manager._TaskManager._is_readable") as mck:
            task_id = TaskId("TASK_id")
        ...
        with mock.patch("pyforge.core.job._job_manager._JobManager._is_readable") as mck:
            job_id = JobId("JOB_id")
        ...
        with mock.patch("pyforge.core.data._data_manager._DataManager._is_readable") as mck:
            dn_id = DataNodeId("DATANODE_id")
        ...
        with mock.patch("pyforge.core.submission._submission_manager._SubmissionManager._is_readable") as mck:
            submission = Submission(scenario.id, scenario._ID_PREFIX, scenario.config_id, "submission_id")
    ...
    def test_is_submittable_is_called(self):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._is_submittable") as mck:
            scenario_id = ScenarioId("SCENARIO_id")
        ...
        with mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._is_submittable") as mck:
            sequence_id = SequenceId("SEQUENCE_id")
        ...
        with mock.patch("pyforge.core.task._task_manager._TaskManager._is_submittable") as mck:
            task_id = TaskId("TASK_id")
    ...
    def test_submit(self, scenario, sequence, task):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._submit") as mck:
            tp.submit(scenario)
            mck.assert_called_once_with(scenario, force=False, wait=False, timeout=None)
        with mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._submit") as mck:
            tp.submit(sequence)
            mck.assert_called_once_with(sequence, force=False, wait=False, timeout=None)
        with mock.patch("pyforge.core.task._task_manager._TaskManager._submit") as mck:
            tp.submit(task)
    ...
    def test_warning_no_core_service_running(self, scenario):
        ...
        with pytest.warns(ResourceWarning) as warning:
            with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._submit"):
                tp.submit(scenario)
    ...
    def test_get_tasks(self):
        with mock.patch("pyforge.core.task._task_manager._TaskManager._get_all") as mck:
            tp.get_tasks()
            mck.assert_called_once_with()

    def test_get_task(self, task):
        with mock.patch("pyforge.core.task._task_manager._TaskManager._get") as mck:
            task_id = TaskId("TASK_id")
            tp.get(task_id)
            mck.assert_called_once_with(task_id)

    def test_task_exists(self):
        with mock.patch("pyforge.core.task._task_manager._TaskManager._exists") as mck:
            task_id = TaskId("TASK_id")
            tp.exists(task_id)
            mck.assert_called_once_with(task_id)

    def test_is_deletable(self, task):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._is_deletable") as mck:
            scenario_id = ScenarioId("SCENARIO_id")
        ...
        with mock.patch("pyforge.core.job._job_manager._JobManager._is_deletable") as mck:
        ...
        with mock.patch("pyforge.core.submission._submission_manager._SubmissionManager._is_deletable") as mck:
            submission = Submission(scenario.id, scenario._ID_PREFIX, scenario.config_id, "submission_id")
    ...
    def test_is_promotable(self):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._is_promotable_to_primary") as mck:
            scenario_id = ScenarioId("SCENARIO_id")
    ...
    def test_delete_scenario(self):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._hard_delete") as mck:
            scenario_id = ScenarioId("SCENARIO_id")
            tp.delete(scenario_id)
            mck.assert_called_once_with(scenario_id)

    def test_delete(self):
        with mock.patch("pyforge.core.cycle._cycle_manager._CycleManager._hard_delete") as mck:
            cycle_id = CycleId("CYCLE_id")
            tp.delete(cycle_id)
            mck.assert_called_once_with(cycle_id)

    def test_get_scenarios(self, cycle):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._get_all") as mck:
            tp.get_scenarios()
            mck.assert_called_once_with()
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._get_all_by_cycle") as mck:
            tp.get_scenarios(cycle)
            mck.assert_called_once_with(cycle)
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._get_all_by_tag") as mck:
            tp.get_scenarios(tag="tag")
            mck.assert_called_once_with("tag")
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._filter_by_creation_time") as mck:
    ...
    def test_get_scenario(self, scenario):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._get") as mck:
            scenario_id = ScenarioId("SCENARIO_id")
            tp.get(scenario_id)
            mck.assert_called_once_with(scenario_id)

    def test_scenario_exists(self):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._exists") as mck:
            scenario_id = ScenarioId("SCENARIO_id")
            tp.exists(scenario_id)
            mck.assert_called_once_with(scenario_id)

    def test_get_primary(self, cycle):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._get_primary") as mck:
            tp.get_primary(cycle)
            mck.assert_called_once_with(cycle)

    def test_get_primary_scenarios(self):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._get_primary_scenarios") as mck:
    ...
    def test_set_primary(self, scenario):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._set_primary") as mck:
            tp.set_primary(scenario)
            mck.assert_called_once_with(scenario)

    def test_tag_and_untag(self, scenario):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._tag") as mck:
            tp.tag(scenario, "tag")
            mck.assert_called_once_with(scenario, "tag")
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._untag") as mck:
            tp.untag(scenario, "tag")
            mck.assert_called_once_with(scenario, "tag")

    def test_compare_scenarios(self, scenario):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._compare") as mck:
    ...
    def test_subscribe_scenario(self, scenario):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._subscribe") as mck:
    ...
    def test_unsubscribe_scenario(self, scenario):
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._unsubscribe") as mck:
    ...
    def test_subscribe_sequence(self, sequence):
        with mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._subscribe") as mck:
    ...
    def test_unsubscribe_sequence(self, sequence):
        with mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._unsubscribe") as mck:
    ...
    def test_delete_sequence(self):
        with mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._hard_delete") as mck:
            sequence_id = SequenceId("SEQUENCE_id")
            tp.delete(sequence_id)
            mck.assert_called_once_with(sequence_id)

    def test_get_sequence(self, sequence):
        with mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._get") as mck:
            sequence_id = SequenceId("SEQUENCE_id")
            tp.get(sequence_id)
            mck.assert_called_once_with(sequence_id)

    def test_get_sequences(self):
        with mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._get_all") as mck:
            tp.get_sequences()
            mck.assert_called_once_with()

    def test_sequence_exists(self):
        with mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._exists") as mck:
            sequence_id = SequenceId("SEQUENCE_id")
            tp.exists(sequence_id)
            mck.assert_called_once_with(sequence_id)

    def test_get_job(self):
        with mock.patch("pyforge.core.job._job_manager._JobManager._get") as mck:
            job_id = JobId("JOB_id")
            tp.get(job_id)
            mck.assert_called_once_with(job_id)

    def test_get_jobs(self):
        with mock.patch("pyforge.core.job._job_manager._JobManager._get_all") as mck:
            tp.get_jobs()
            mck.assert_called_once_with()

    def test_job_exists(self):
        with mock.patch("pyforge.core.job._job_manager._JobManager._exists") as mck:
            job_id = JobId("JOB_id")
            tp.exists(job_id)
            mck.assert_called_once_with(job_id)

    def test_delete_job(self, task):
        with mock.patch("pyforge.core.job._job_manager._JobManager._delete") as mck:
    ...
    def test_delete_jobs(self):
        with mock.patch("pyforge.core.job._job_manager._JobManager._delete_all") as mck:
            tp.delete_jobs()
            mck.assert_called_once_with()

    def test_get_latest_job(self, task):
        with mock.patch("pyforge.core.job._job_manager._JobManager._get_latest") as mck:
            tp.get_latest_job(task)
            mck.assert_called_once_with(task)

    def test_cancel_job(self):
        with mock.patch("pyforge.core.job._job_manager._JobManager._cancel") as mck:
            tp.cancel_job("job_id")
            mck.assert_called_once_with("job_id")

    def test_get_submissions(self):
        with mock.patch("pyforge.core.submission._submission_manager._SubmissionManager._get_all") as mck:
            tp.get_submissions()
            mck.assert_called_once_with()

    def test_get_submission(self, task):
        with mock.patch("pyforge.core.submission._submission_manager._SubmissionManager._get") as mck:
            submission_id = SubmissionId("SUBMISSION_id")
            tp.get(submission_id)
            mck.assert_called_once_with(submission_id)

    def test_get_latest_submission(self, task):
        with mock.patch("pyforge.core.submission._submission_manager._SubmissionManager._get_latest") as mck:
    ...
    def test_get_data_node(self, data_node):
        with mock.patch("pyforge.core.data._data_manager._DataManager._get") as mck:
            tp.get(data_node.id)
            mck.assert_called_once_with(data_node.id)

    def test_get_data_nodes(self):
        with mock.patch("pyforge.core.data._data_manager._DataManager._get_all") as mck:
            tp.get_data_nodes()
            mck.assert_called_once_with()

    def test_data_node_exists(self):
        with mock.patch("pyforge.core.data._data_manager._DataManager._exists") as mck:
            data_node_id = DataNodeId("DATANODE_id")
            tp.exists(data_node_id)
            mck.assert_called_once_with(data_node_id)

    def test_get_cycles(self):
        with mock.patch("pyforge.core.cycle._cycle_manager._CycleManager._get_all") as mck:
            tp.get_cycles()
            mck.assert_called_once_with()

    def test_cycle_exists(self):
        with mock.patch("pyforge.core.cycle._cycle_manager._CycleManager._exists") as mck:
            cycle_id = CycleId("CYCLE_id")
            tp.exists(cycle_id)
            mck.assert_called_once_with(cycle_id)
    ...
    def test_create_global_data_node(self):
        ...
        with mock.patch("pyforge.core.data._data_manager._DataManager._create_and_set") as dn_create_mock:
            with mock.patch("pyforge.core.orchestrator.Orchestrator._manage_version_and_block_config") as mv_mock:
    ...
    def test_create_scenario(self):
        ...
        with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._create") as mck:
            with mock.patch("pyforge.core.orchestrator.Orchestrator._manage_version_and_block_config") as mv_mock:
