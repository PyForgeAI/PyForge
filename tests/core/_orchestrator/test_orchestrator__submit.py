...
from pyforge.core import Orchestrator, Scenario, Scope, Task, pyforge
...
from pyforge.core.config import JobConfig
from pyforge.core.data import PickleDataNode
...
from pyforge.core.submission.submission_status import SubmissionStatus
...
def create_scenario():
    ...
    return pyforge.create_scenario(sc_conf)
...
def test_submit_scenario_with_callbacks_and_force_and_wait():
    ...
    with mock.patch("pyforge.core._orchestrator._orchestrator._Orchestrator._wait_until_job_finished") as mck:
...
def test_submit_submittable_generate_unique_submit_id():
    ...
    jobs_1 = pyforge.submit(scenario).jobs
    jobs_2 = pyforge.submit(scenario).jobs
    assert len(jobs_1) == 2
    assert len(jobs_2) == 2
...
def test_submit_duration_development_mode():
    ...
    scenario = Scenario("scenario", {task_1, task_2}, {})
    _ScenarioManager._set(scenario)
    submission = pyforge.submit(scenario)
    jobs = submission.jobs
    orchestrator.stop()
