...
from pyforge.core._init_version import _read_version
from pyforge.core.config.core_section import CoreSection
from pyforge.core.exceptions import ConfigCoreVersionMismatched
...
def patch_core_version(mock_core_version: str):
    with mock.patch("pyforge.core.config.core_section._read_version") as mock_read_version:
...
class TestCoreVersionInCoreSectionConfig:
    ...
    def test_run_core_app_with_different_pyforge_core_version_in_development_mode(self):
    ...
    def test_run_core_app_with_different_pyforge_core_version_in_experiment_mode(self, caplog):
        ...
        with mock.patch("sys.argv", ["prog", "--experiment", "1.0"]):
            with pytest.raises(SystemExit):
                run_application()
        assert (
            f"The version {self.major}.{self.minor}.{self.patch} of PyForge's entities does not match version "
            f"of the PyForge Version management {self.major}.{int(self.minor) + 1}.{self.patch}.dev0"
        ) in caplog.text
