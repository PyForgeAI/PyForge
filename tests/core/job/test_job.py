...
from pyforge.core import JobId, TaskId
from pyforge.core._orchestrator._abstract_orchestrator import _AbstractOrchestrator
from pyforge.core._orchestrator._dispatcher._development_job_dispatcher import _DevelopmentJobDispatcher
from pyforge.core._orchestrator._dispatcher._standalone_job_dispatcher import _StandaloneJobDispatcher
from pyforge.core._orchestrator._orchestrator_factory import _OrchestratorFactory
from pyforge.core.config.job_config import JobConfig
from pyforge.core.data.in_memory import InMemoryDataNode
...
from pyforge.core.job.status import Status
...
from pyforge.core.task.task import Task
...
def test_handle_exception_in_input_data_node(task_id, job_id):
    ...
    job = _JobManager._get(job_id)
    assert job.is_failed()
    assert "pyforge.core.exceptions.exceptions.NoData" in str(job.stacktrace[0])
...
def test_handle_exception_in_ouptut_data_node(replace_in_memory_write_fct, task_id, job_id):
    ...
    assert job.is_failed()
    assert "pyforge.core.exceptions.exceptions.DataNodeWritingError" in str(job.stacktrace[0])
...
def test_is_deletable():
    with mock.patch("pyforge.core.job._job_manager._JobManager._is_deletable") as mock_submit:
