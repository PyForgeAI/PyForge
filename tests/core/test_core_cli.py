...
from pyforge.core import Orchestrator, pyforge
...
from pyforge.core.common._utils import _load_fct
...
from pyforge.core.exceptions.exceptions import NonExistingVersion
...
def test_orchestrator_cli_experiment_mode_with_force_version(init_config):
    with patch("sys.argv", ["prog", "--experiment", "2.1", "--pyforge-force"]):
        init_config()
        orchestrator = Orchestrator()
        orchestrator.run()
        assert Config.core.mode == "experiment"
...
def test_force_override_experiment_version():
    ...
    # Without --pyforge-force parameter, a SystemExit will be raised
    ...
    # With --pyforge-force parameter
    with patch("sys.argv", ["prog", "--experiment", "1.0", "--pyforge-force"]):
        orchestrator = Orchestrator()
        orchestrator.run()
        ver_2 = _VersionManager._get_latest_version()
        assert ver_2 == "1.0"
