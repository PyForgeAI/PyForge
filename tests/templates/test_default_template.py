...
def test_default_answer(tmpdir):
    cookiecutter(
        template="pyforge/templates/default",
        output_dir=str(tmpdir),
        no_input=True,
    )

    assert os.listdir(tmpdir) == ["pyforge_application"]
    assert sorted(os.listdir(os.path.join(tmpdir, "pyforge_application"))) == sorted(["requirements.txt", "main.py"])

    pyforge_path = os.getcwd()
    stdout = _run_template(pyforge_path, os.path.join(tmpdir, "pyforge_application"), "main.py")

    # Assert the message when the application is run successfully is in the stdout
    assert "[PyForge][INFO]  * Server starting on" in stdout
...
def test_main_file_with_and_without_extension(tmpdir):
    ...
    assert sorted(os.listdir(os.path.join(tmpdir, "pyforge_application"))) == sorted(["requirements.txt", "app.py"])
...
def test_with_orchestrator_service(tmpdir):
    ...
    assert sorted(os.listdir(os.path.join(tmpdir, "pyforge_application"))) == sorted(
        ["requirements.txt", "main.py", "configuration", "algorithms"]
    )
    with open(os.path.join(tmpdir, "pyforge_application", "main.py")) as main_file:
    ...
    # Assert the message when the application is run successfully is in the stdout
    assert "[PyForge][INFO]  * Server starting on" in stdout
    assert "[PyForge][INFO] Development mode: " in stdout
...
def test_multipage_gui_template(tmpdir):
    ...
    pyforge_path = os.getcwd()
    stdout = _run_template(pyforge_path, os.path.join(tmpdir, "foo_app"), "main.py")
    assert "[PyForge][INFO]  * Server starting on" in stdout
