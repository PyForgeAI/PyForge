...
from pyforge.common.config import Config, _inject_section
from pyforge.common.config._config import _Config
from pyforge.common.config._config_comparator._config_comparator import _ConfigComparator
from pyforge.common.config._serializer._toml_serializer import _TomlSerializer
...
def remove_subparser(name: str) -> None:
    ...
    _PyForgeParser._sub_pyforgeparsers.pop(name, None)

    if _PyForgeParser._subparser_action:
        _PyForgeParser._subparser_action._name_parser_map.pop(name, None)

        for action in _PyForgeParser._subparser_action._choices_actions:
            if action.dest == name:
                _PyForgeParser._subparser_action._choices_actions.remove(action)
...
def clean_argparser() -> t.Callable:
    ...
    def _clean_argparser() -> None:
        _PyForgeParser._parser = argparse.ArgumentParser(conflict_handler="resolve")
        _PyForgeParser._subparser_action = None
        _PyForgeParser._arg_groups = {}
        subcommands = list(_PyForgeParser._sub_pyforgeparsers.keys())
