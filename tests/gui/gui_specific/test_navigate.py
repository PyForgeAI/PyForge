...
from pyforge.gui import Gui, Markdown, State, navigate
...
def test_navigate(gui: Gui, helpers):
    ...
    with warnings.catch_warnings(record=True):
        ...
        client.get(f"/pyforge-jsx/test/?client_id={sid}")
...
def test_on_navigate_to_inexistant(gui: Gui, helpers):
    ...
    with warnings.catch_warnings(record=True) as records:
        ...
        sid = helpers.create_scope_and_get_sid(gui)
        client.get(f"/pyforge-jsx/test?client_id={sid}")
        warns = helpers.get_pyforge_warnings(records)
        assert len(warns) == 1
...
def test_on_navigate_to_existant(gui: Gui, helpers):
    ...
    with warnings.catch_warnings(record=True):
        ...
        # Get the jsx once so that the page will be evaluated -> variable will be registered
        sid = helpers.create_scope_and_get_sid(gui)
        content = client.get(f"/pyforge-jsx/test1?client_id={sid}")
