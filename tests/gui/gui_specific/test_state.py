...
def test_state(gui: Gui):
    ...
    with gui.get_flask_app().app_context():
        ...
        with pytest.raises(Exception) as e:
            _ = state._pyforge_p1
        assert e.value.args[0] == "Variable '_pyforge_p1' is protected and is not accessible."

        with pytest.raises(Exception) as e:
            state._pyforge_p1 = 10
        assert e.value.args[0] == "Variable '_pyforge_p1' is not accessible."

        assert state._get_placeholder("_pyforge_p1") is None

        state._set_placeholder("_pyforge_p1", 10)

        assert state._get_placeholder("_pyforge_p1") == 10
