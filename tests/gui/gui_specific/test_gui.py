...
from pyforge.gui.utils import _get_module_name_from_frame, _PyForgeContent
    ...
    with gui.get_flask_app().app_context():
        with gui._set_locals_context(_get_module_name_from_frame(frame)) if frame else nullcontext():
            with pytest.raises(NameError):
                res = gui._get_real_var_name(f"{_PyForgeContent.get_hash()}_var")
...
def test__tbl_cols(gui: Gui):
    ...
    with gui.get_flask_app().app_context():
        ...
        assert repr(res) == "PyForge: Do not update"
