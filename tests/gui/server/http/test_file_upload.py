...
from pyforge.gui.data.data_scope import _DataScopes
from pyforge.gui.utils import _get_non_existent_file_path
...
def test_file_upload_no_varname(gui: Gui, helpers):
    ...
    with pytest.warns(UserWarning):
        ret = flask_client.post(f"/pyforge-uploads?client_id={sid}")
...
def test_file_upload_no_blob(gui: Gui, helpers):
    ...
    sid = helpers.create_scope_and_get_sid(gui)
    with pytest.warns(UserWarning):
        ret = flask_client.post(f"/pyforge-uploads?client_id={sid}", data={"var_name": "varname"})
        assert ret.status_code == 400
...
def test_file_upload_no_filename(gui: Gui, helpers):
    ...
    sid = helpers.create_scope_and_get_sid(gui)
    with pytest.warns(UserWarning):
        ret = flask_client.post(f"/pyforge-uploads?client_id={sid}", data={"var_name": "varname", "blob": file})
        assert ret.status_code == 400
...
def test_file_upload_simple(gui: Gui, helpers):
    ...
    ret = flask_client.post(
        f"/pyforge-uploads?client_id={sid}",
...
def test_file_upload_multiple(gui: Gui, helpers):
    ...
    ret = flask_client.post(
        f"/pyforge-uploads?client_id={sid}", data={"var_name": var_name, "blob": file}, content_type="multipart/form-data"
    )
    assert ret.status_code == 200
