...
from pyforge.gui import Gui, Html
from pyforge.gui.server import _Server
...
<body>
    <pyforge:text id="text1">Hey</pyforge:text>
    <pyforge:text id="text2" class="custom-text">There</pyforge:text>
</body>
</html>"""
...
def test_html_render_bind_assets(page: "Page", gui: Gui, helpers, e2e_base_url, e2e_port):
    ...
    helpers.run_e2e(gui)
    assert ".pyforge-text" in urlopen(
...
def test_html_render_path_mapping(page: "Page", gui: Gui, helpers, e2e_base_url, e2e_port):
    ...
    helpers.run_e2e(gui)
    assert ".pyforge-text" in urlopen(f"http://127.0.0.1:{e2e_port}{e2e_base_url}/style/style.css").read().decode("utf-8")
    page.goto("./page1")
    page.expect_websocket()
    page.wait_for_selector("#text1")
    retry = 0
