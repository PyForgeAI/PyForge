...
from pyforge.core import Cycle, CycleId, Job, JobId, Scenario, Task
...
from pyforge.gui import Gui, State
from pyforge.gui_core._context import _GuiCoreContext
from pyforge.gui_core._utils import _ClientStatus
...
class TestGuiCoreContext_is_readable:
    ...
    def test_scenario_adapter(self):
        with patch("pyforge.gui_core._context.core_get", side_effect=mock_core_get):
            ...
            with patch("pyforge.gui_core._context.is_readable", side_effect=mock_is_readable_false):
                outcome = gui_core_context.scenario_adapter(a_scenario)
    ...
    def test_submission_status_callback(self):
        with patch("pyforge.gui_core._context.core_get", side_effect=mock_core_get) as mockget:
            mockget.reset_mock()
            mockGui = Mock(Gui)
    ...
    def test_act_on_jobs(self):
        with (
            patch("pyforge.gui_core._context.core_get", side_effect=mock_core_get),
            patch("pyforge.gui_core._context.is_deletable", side_effect=mock_is_true),
        ):
            gui_core_context = _GuiCoreContext(Mock())
            assign = Mock()
