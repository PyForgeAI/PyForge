...
from pyforge import DataNode, Gui, Scope
...
from pyforge.core.reason import Reason, ReasonCollection
...
class TestGuiCoreContext_update_data:
    ...
    def test_do_not_update_data_if_not_readable(self):
        with patch("pyforge.gui_core._context.is_readable", side_effect=is_false):
            with patch("pyforge.gui_core._context.core_get", side_effect=core_get) as mock_core_get:
    ...
    def test_do_not_update_data_if_not_editable(self):
        with patch("pyforge.gui_core._context.is_readable", side_effect=is_true):
            with patch("pyforge.gui_core._context.is_editable", side_effect=is_false):
                with patch("pyforge.gui_core._context.core_get", side_effect=core_get) as mock_core_get:
                    with patch.object(DataNode, "write") as mock_write:
                        mockGui = Mock(Gui)
                        mockGui._get_client_id = lambda: "a_client_id"
    ...
    def test_write_str_data_with_editor_and_comment(self):
        with patch("pyforge.gui_core._context.is_readable", side_effect=is_true):
            with patch("pyforge.gui_core._context.is_editable", side_effect=is_true):
                with patch("pyforge.gui_core._context.core_get", side_effect=core_get) as mock_core_get:
                    with patch.object(DataNode, "write") as mock_write:
                        mockGui = Mock(Gui)
                        mockGui._get_client_id = lambda: "a_client_id"
