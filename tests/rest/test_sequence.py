...
def test_get_sequence(client, default_sequence):
    ...
    with mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._get") as manager_mock:
...
def test_delete_sequence(client):
    ...
    with (
        mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._delete"),
        mock.patch("pyforge.core.sequence._sequence_manager._SequenceManager._get"),
    ):
        # test get_sequence
...
def test_create_sequence(client, default_scenario):
    ...
    with mock.patch("pyforge.core.scenario._scenario_manager._ScenarioManager._get") as config_mock:
...
def test_get_all_sequences(client, default_scenario_config_list):
    for ds in range(10):
        with mock.patch("pyforge.rest.api.resources.scenario.ScenarioList.fetch_config") as config_mock:
