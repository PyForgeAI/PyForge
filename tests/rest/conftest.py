...
from pyforge.core import Cycle, DataNodeId, Job, JobId, Scenario, Sequence, Task
...
from pyforge.rest.app import create_app
...
def default_datanode_config():
    return Config.configure_data_node(f"pyforge_{uuid.uuid4().hex}", "in_memory", Scope.SCENARIO)
...
def default_scenario_config_list():
    configs = []
    for _ in range(10):
        task_config = Config.configure_task(f"pyforge_{uuid.uuid4().hex}", print)
        scenario_config = Config.configure_scenario(
            f"pyforge_{uuid.uuid4().hex}",
            [task_config],
        )
